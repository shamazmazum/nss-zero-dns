...
int verify_name_allowed_with_soa(const char* name, FILE* zerodns_allow_file) {
    switch (verify_name_allowed(name, zerodns_allow_file)) {
...
verify_name_result_t verify_name_allowed(const char* name,
                                         FILE* zerodns_allow_file) {
    assert(name);

    if (zerodns_allow_file) {
        int valid = 0;

        while (!feof(zerodns_allow_file)) {
            char ln[128], ln2[129], *t;

            if (!fgets(ln, sizeof(ln), zerodns_allow_file))
                break;
